...
const GOOGLE_KEY = process.env.REACT_APP_GOOGLE_API_KEY;
    ...
    export let getZip = async (latitude, longitude) => {
        return fetch("https://maps.googleapis.com/maps/api/geocode/json?latlng=" + latitude.toString() + ',' + longitude.toString() + "&key=" + GOOGLE_KEY)
    ...
    export let getLatLong = async (userZip) => {
        ...
        return fetch("https://maps.googleapis.com/maps/api/geocode/json?address=" + userZip.toString() + 'components=postal_code:' + userZip.toString() +"&key=" + GOOGLE_KEY)
            .then(response => response.json())
            .then(jsonResponse => {
                let data = jsonResponse;
    ...
    export let useMyZip = (userZip) => {
        let location = getLatLong(userZip);
        return(location);
   }
